...
# def pre_process(eng, may, reverse=False):
#     english = []
#     maya = []
#     for i, j in zip(eng, may):
#         lines = open(i, encoding='utf-8').read().strip().split('\n')
#         english.append(lines)
#
#         lines = open(j, encoding='utf-8').read().strip().split('\n')
#         maya.append(lines)
#
#     english = [i for sublist in english for i in sublist]
#     maya = [i for sublist in maya for i in sublist]
#
#     L1 = [normalize_string(s) for s in english]
#     L2 = [normalize_string(s) for s in maya]
#
#     l1 = [i for i, e in enumerate(L1) if e == ' ']
#     L11 = [j for i, j in enumerate(L1) if i not in l1]
#     L22 = [j for i, j in enumerate(L2) if i not in l1]
